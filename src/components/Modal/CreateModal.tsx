...
function CreateModal({ isOpen, onClose }: CreateModalProps) {
  ...
  const handleSubmit = async () => {
    await walletContext.getWeb3Provider();
    ...
    <Modal isOpen={isOpen} onClose={onClose}>
      ...
      <form >
        <ModalContent>
          ...
          <ModalBody px="20px" py="30px">
              ...
              <Grid templateColumns='repeat(4, 100px)' gap={4}>
                {
                  services.map((service, i) => (
                    <Image
                      ...
                      boxShadow={service === selectedService ? '0px 0px 4px 3px #E50914' : '0px 0px 4px 1px rgba(0, 0, 0, 0.25)'}
                      _hover={{ opacity: 0.4 }}
            ...
            <FormControl mb={2}>
              ...
              <NumberInput min={0} precision={2}>
                <NumberInputField placeholder='In Matic' value={price} onChange={(event) => setPrice(event.target.value)} required />
              </NumberInput>
            </FormControl>
            ...
            <FormControl mb={2}>
              ...
              <NumberInput min={0} precision={2}>
                <NumberInputField placeholder='In Matic' value={renewalFee} onChange={(event) => setRenewalFee(event.target.value)} required />
              </NumberInput>
            </FormControl>
            ...
            <FormControl mb={4}>
              ...
              <Flex justifyContent="space-between">
                <Input placeholder='Username / Email' value={username} onChange={(event) => setUsername(event.target.value)} required me={2} />
                <Input placeholder='Password' value={password} onChange={(event) => setPassword(event.target.value)} required ms={2} />
              </Flex>

            </FormControl>

            <Flex flexDirection="column">
              <Text fontSize="1xl" mb={3}>User guidelines</Text>
          ...
          <ModalFooter p={0}>
            <Button type='submit' width="100%" colorScheme="green" borderRadius="none" onClick={handleSubmit}>
              Create
            </Button>
          </ModalFooter>
